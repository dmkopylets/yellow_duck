...
composer-i:
	$(EXEC_PHP) sh -c "cd api; composer install"

clear-cache:
	$(EXEC_PHP) sh -c "cd api; php artisan route:clear && php artisan cache:clear &&  php artisan config:cache && php artisan view:clear && php artisan route:clear && php artisan optimize:clear"
